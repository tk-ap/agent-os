...
Before executable approval, apply the merged zero-trust skill supply-chain model in `policies/SKILL_TRUST_POLICY.md` and `docs/security/SKILL_ADMISSION_WORKFLOW.md`:

- immutable source capture;
- optional scanner evidence from an admitted security tool;
...
`registry/skill-admissions.yaml` is the admission ledger. Benchmark usefulness cannot substitute for security admission.
