...
Quality railguards prevent agent-generated work from becoming generic, inflated, fabricated, overbuilt, or superficially verified. They are a quality control layer, not an aesthetic authority and not a substitute for product direction.
...
7. **Direction over anti-slop monoculture** — product-local art direction, voice, interaction language, and repository conventions outrank generic anti-slop preferences unless they conflict with safety, truth, accessibility, or explicit acceptance criteria.
...
2. Run available mechanical checks that already belong to the repository.
3. Apply `skills/owned/quality-railguards/SKILL.md` to the changed scope.
4. Load only an approved specialist anti-slop skill when it adds material value beyond the owned railguard.
5. Fix blockers and clearly justified quality failures; do not churn subjective preferences.
6. Compose with `skills/owned/end-to-end-verification/SKILL.md` when the promise crosses runtime, deployment, UI, data, authorization, or integration boundaries.
7. Capture evidence and classify the result as `pass`, `partial`, `failed`, or `blocked`.
8. When aesthetic or experiential acceptance is materially subjective, preserve a human acceptance gate. Automated anti-slop review cannot certify taste.
...
External anti-slop skills are capability supply, not Agent OS policy. They remain subordinate to:
...
Never allow an upstream install wizard, `load always` instruction, tool declaration, or mutable latest version to bypass Agent OS skill review and pinning.
